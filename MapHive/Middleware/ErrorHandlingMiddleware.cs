...
public class ErrorHandlingMiddleware(RequestDelegate next)
{
        ...
        IConfigurationSingleton _configurationSingleton)
    {
        ...
        catch (Exception ex)
        {
            try
            {
                if (await _configurationSingleton.DevelopmentModeAsync() || (_userContextService.IsAuthenticated && _userContextService.IsAdminOrThrow))
                {
                    int logId = await _logManagerService.LogAsync(
